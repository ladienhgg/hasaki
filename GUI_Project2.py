...
# Thiết kế menu
menu = ["Introduction", "Sentiment Analysis", "Content based filtering", "Colaborative system"]
choice = st.sidebar.selectbox('Menu', menu)
st.sidebar.write("""#### Học viên thực hiện:
                 Lã Đình Điền""")
st.sidebar.image("Anh.jpg", width=150)
...
if choice == 'Introduction':    
    ...
    df_san_pham = pd.read_csv('San_pham.csv')
    df_khach_hang = pd.read_csv('Khach_hang.csv')
    df_tong_hop = pd.merge(df_danh_gia, df_khach_hang, on='ma_khach_hang')
    df_tong_hop = pd.merge(df_tong_hop, df_san_pham, on='ma_san_pham')
    # Lấy 50 dòng dữ liệu
    random_khach_hang = df_khach_hang.head(n=50)
        #print(random_danh_gia)

    st.session_state.random_khach_hang = random_khach_hang


    # Open and read file to cosine_sim_new
    with open('svdpp_model.joblib', 'rb') as f:
    ...
    # Theo cách cho người dùng chọn sản phẩm từ dropdown
    # Tạo một tuple cho mỗi sản phẩm, trong đó phần tử đầu là tên và phần tử thứ hai là ID
    user_options = [(row['ho_ten'], row['ma_khach_hang']) for index, row in st.session_state.random_khach_hang.iterrows()]
    st.session_state.random_khach_hang
    # Tạo một dropdown với options là các tuple này
    ...
    # Display the selected product
    st.write("Bạn đã chọn:", selected_user)

    # Cập nhật session_state dựa trên lựa chọn hiện tại
    st.session_state.selected_ma_khach_hang = selected_user[1]
    # userId has seen some product:
    userId = selected_user[1]
    df_select = df_danh_gia[(df_danh_gia['ma_khach_hang'] == userId) & (df_danh_gia['so_sao'] >=3)]
    ...
    if st.session_state.selected_ma_khach_hang:
        st.write("ma_khach_hang: ", st.session_state.selected_ma_khach_hang)
        # Hiển thị thông tin sản phẩm được chọn
        #selected_user = df_products[df_products['ma_san_pham'] == st.session_state.selected_ma_san_pham]
